...
void setup()
{
  ...
  gripper_left_motor = new LokSrc::MotorController(GRIPPER_L_MOTOR_DRIVER_IN1, GRIPPER_L_MOTOR_DRIVER_IN2, GRIPPER_L_MOTOR_DRIVER_IN3, 
    GRIPPER_L_MOTOR_DRIVER_IN4, GRIPPER_L_FW_LIMIT_SWITCH_, GRIPPER_L_BW_LIMIT_SWITCH_);
  gripper_right_motor = new LokSrc::MotorController(GRIPPER_R_MOTOR_DRIVER_IN1, GRIPPER_R_MOTOR_DRIVER_IN2, GRIPPER_R_MOTOR_DRIVER_IN3, 
    GRIPPER_R_MOTOR_DRIVER_IN4, GRIPPER_R_FW_LIMIT_SWITCH_, GRIPPER_R_BW_LIMIT_SWITCH_);
...
void loop()
{
  ...
  uint16_t number_data = 0;
  bool clicked_state = controllers.getRCdata(goal_velocity_from_rc100, number_data);
  if (clicked_state == true)  
    tTime[6] = millis();

  updateLeds(number_data);
